...
pacman::p_load(tidyverse, here, glue, scales, ggridges, dsbox, fs, janitor, ggforce, palmerpenguins, showtext)
...
** confused with scale_x_continous(n.breaks) was not working. Consulted Ai which pointed me to scale_x_date. Code created by self through referring to documentation https://ggplot2.tidyverse.org/reference/scale_date.html 
*** inspired to use negative placement values by https://stackoverflow.com/questions/25401111/left-adjust-title-in-ggplot2-or-absolute-position-for-ggtitle 
...
bounded_recession
```
* inspired to make separate variables from https://www.r-bloggers.com/2024/02/how-to-check-if-date-is-between-two-dates-in-r/
...
subset_med_house <- median_housing |>
  ...
  mutate(
    year = year(date),
    quarter = quarter(date, type = "year.quarter"), #from lubridate documentation
  )
subset_med_house
```
```{r}
subset_med_house |>
  ggplot(aes(x = quarter, y = price)) +
...
penguins |>
  ...
  theme(
    ...
    plot.title = element_text(color = "#E92EFB", family = "mono", size = 19.5, hjust = -1),
